...
export default function SkillGapAnalyzer() {
  ...
  // Parse skills from raw input (comma separated)
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(e.detail || 'Failed to analyze gaps');
      }
  ...
  // Count critical and important gaps
    ...
    <div style={{ width: '100%', maxWidth: 860, margin: '0 auto', animation: 'fadeUp 0.4s ease' }}>
      ...
      {step === 1 && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 32 }}>
          <label className="field-label">Your current skills</label>
          <textarea
            className="text-field"
            value={rawInput}
            onChange={e => setRawInput(e.target.value)}
            placeholder="python, sql, machine learning, docker, react, aws…"
            rows={4}
          />
          <p className="field-hint">Separate with commas · <strong>ml</strong> and <strong>ci/cd</strong> auto-expand</p>
          {skills.length > 0 && (
            <div className="tags-container" style={{ marginTop: 16 }}>
              {skills.map(s => (
      ...
      {step === 2 && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 32 }}>
          <div style={{ marginBottom: 20, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {skills.length > 6 && <span style={{ color: 'var(--text-muted)', fontSize: 12, fontFamily: 'var(--font-mono)' }}>+{skills.length - 6} more</span>}
            <button 
              onClick={() => setStep(1)} 
              style={{ 
          ...
          <div style={{ 
            ...
            {filteredDomains.length > 0 ? (
              ...
              <div style={{ color: 'var(--text-muted)', textAlign: 'center', padding: 40, fontFamily: 'var(--font-mono)', fontSize: 13 }}>
          ...
          >
            {loading ? 'Analyzing gaps…' : `Analyze gaps for "${domain || 'selected domain'}"`}
          </button>
        </div>
      )}
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24, marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, fontFamily: 'var(--font-mono)', fontSize: 12 }}>
              <span style={{ color: 'var(--text-secondary)' }}>
                Your skill coverage for <strong style={{ color: 'var(--amber)' }}>{result.domain}</strong>
              </span>
              <span style={{ color: 'var(--text-muted)' }}>
                {result.input_skills.length} / {result.input_skills.length + result.missing_skills.length} skills
              </span>
            </div>
            <div style={{ height: 8, background: 'var(--bg-elevated)', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{
                height: '100%',
                width: `${coverage}%`,
                background: 'linear-gradient(90deg, var(--amber), #f7b84a)',
                borderRadius: 8,
                transition: 'width 1s cubic-bezier(0.16,1,0.3,1)'
              }} />
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
              {result.input_skills.slice(0, 12).map(s => (
                <span key={s} className="skill-tag" style={{ background: 'var(--green-dim)', color: 'var(--green)', borderColor: 'rgba(52,211,153,0.2)' }}>
                  ✓ {s}
                </span>
              ))}
              {result.input_skills.length > 12 && (
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)' }}>
                  +{result.input_skills.length - 12} more
                </span>
              )}
            </div>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: m.color, width: 56, textAlign: 'right' }}>
